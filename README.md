...
- **Umbriel** — scrolling Wayland compositor: horizontal strips on landscape monitors and vertical strips on HDMI-A-1 / rotated monitors. Landscape outputs use dynamic workspaces (bare digits select a position on the output under the pointer) plus four persistent named workspaces: `STEAM`/`GAME` follow the mode's gaming output (DP-2 in desktop mode, DP-3 in work mode), `MAIL` stays on DP-3, and `MEDIA` on the portrait. Portrait `MEDIA` opens Discord above Spotify at 75/25; a session listener restores that arrangement when either app reopens while allowing manual resizing. Window rules route Steam to `STEAM`, games to `GAME`, Thunderbird to `MAIL`, and Discord/Spotify to `MEDIA`; Firefox and other apps open on the active workspace. Mod+1–0 selects local positions 1–10 with Shift sending windows there; Mod+S/G/T jump to the named workspaces and Mod+Shift+S/G/T/D send windows there. The work-mode toggle disables DP-2 and migrates the `STEAM`/`GAME` windows to the same-named workspaces on DP-3 (anonymous windows are left on DP-3), moving them back on return while `MAIL` stays put. Game windows open fullscreen.
