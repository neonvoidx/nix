...
- **Umbriel** — scrolling Wayland compositor: horizontal strips on landscape monitors and vertical strips on HDMI-A-1 / rotated monitors. Portrait workspace 13 opens Discord above Spotify at 75/25; a session listener restores that arrangement when either app reopens while allowing manual resizing. During the first 60 seconds of the session, Firefox and Thunderbird start on workspace 1 on DP-3. Steam always opens on workspace 2 and games on workspace 3, both on DP-2 normally or DP-3 in work mode. Other apps open without fixed workspace routing; Discord/Spotify retain their portrait placement. Desktop mode statically binds the numbered workspaces across the landscape outputs (DP-2 owns 1,3–11 and DP-3 owns 2,12); Mod+1–0 selects local positions 1–10, with Shift sending windows there. The work-mode toggle disables DP-2 and reassigns workspaces 1–12 to DP-3; the script moves each DP-2 window to the same-named workspace on DP-3, and moves windows in `UMBRIEL_MAIN_WORKSPACES` back to DP-2 when returning. Game windows open fullscreen.
