...
All workspaces are dynamic and output-local. Discord and Spotify are the sole placement exception when a `host.monitors.portrait` output exists: both open on that output's dynamic workspace 1 in distinct named scrolling lanes. `wait-for-discord-and-move.sh` is a session-long window listener that keeps Discord above Spotify and reapplies 75%/25% primary extents after either window is replaced or restored. No static workspace, work-mode, monitor-layout, or focus helper scripts remain.

Keybinds: Mod+1–0 select local positions 1–10 and Mod+Shift+1–0 send the window there. Mod+H/J/K/L and the matching arrows use `window-focus-or-output-*`; Mod+Shift+H/J/K/L and matching arrows use `window-move-or-output-*`, so both follow screen directions within scrolling strips and cross to an adjacent output at an edge. Mod+Home/End and Mod+Shift+Home/End walk dynamic workspaces and send windows there; Mod+Grave focuses the last workspace. Mod+=/- resize primary extent by 5%, Mod+R cycles 25/50/75/100%, Mod+C centers a floating window, and Mod+Shift+Space toggles floating. Mod+wheel switches workspaces. Toggles, launches, screenshots, close/quit/lock, direct workspace selection and sending, centering, Alt+Tab, size cycling, and track changes run once per press (`repeat = false`); directional focus/movement, incremental resizing, volume, brightness, and normal typing retain repeat.
