...
Desktop mode statically binds the numbered workspaces across the two landscape outputs (DP-2/main owns 1,3–11 and DP-3/secondary owns 2,12); HDMI-A-1/portrait retains workspace 13. Work mode disables the main output and reassigns workspaces 1–12 to DP-3 so the numbering stays usable. During Umbriel’s first 60 seconds, Firefox and Thunderbird open on workspace 1; afterward they use normal placement. Steam always opens on workspace 2 and games on workspace 3: DP-2/main in desktop mode, DP-3/secondary in work mode. Other editors, launchers, and apps have no fixed workspace rules; Discord and Spotify retain the intentional portrait placement. Explicit output rules route Firefox/Thunderbird to DP-3 and Steam/games to the mode’s gaming output. Game rules open fullscreen, including Gamescope and Godot debug windows.
...
The module generates `config.toml`, `config-desktop.toml`, `config-work.toml`, and `layout.env` (which now also carries `UMBRIEL_MAIN_WORKSPACES`). Toggling copies the selected mode over the live HM symlink and reloads Umbriel. The next Home Manager switch reinstalls the desktop config; the saved mode marker alone does not override that config. The desktop workspace inventories (`mainWorkspaces` / `workWorkspaces`) are defined once in the module and drive both the generated `[output]` sections and `layout.env`.
...
- `toggle-work-mode.sh [desktop|work|toggle]` loads `layout.env` before snapshotting, joins window workspace IDs to workspace names, reloads the selected config, then reconciles windows across the per-output workspaces by name. Entering work mode (which sets `enabled = false` on DP-2 and reassigns 1–12 to DP-3) moves every window that was on DP-2 to the same-named workspace on DP-3. Leaving work mode re-enables DP-2 and moves every DP-3 window whose name is in `UMBRIEL_MAIN_WORKSPACES` back to the same-named workspace on DP-2; DP-3's own workspaces 2/12 stay put. Because Umbriel workspaces are per-output, this name-based migration — not the compositor — owns the round-trip, and a window opened on a reassigned workspace during work mode migrates to DP-2 on toggle-off. Discord/Spotify stay on the portrait. A failed reload restores the actual previous config and leaves the saved mode unchanged.
